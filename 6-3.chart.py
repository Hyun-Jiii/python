...
sel = st.radio('총매출 대비 분석을 원하는 항목을 선택하세요.',
               ('판매량', '매출원가', '수익'))
  
fig = alt.Chart(fi).mark_circle().encode(x='총매출', y=sel, color='상품', size='판매량').properties(width=650, height=450)
...
titanic = pd.read_csv('https://raw.githubusercontent.com/huhshin/streamlit/master/data_titanic.csv', encoding='cp949')
